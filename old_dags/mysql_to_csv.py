...
current_date = date.today().isoformat()  #year-month-day format
today = date.today() 
...
# 	#==========================================part-1===========================================
# 	# bash_cmd=f"~/.virtualenvs/tap-mysql/bin/tap-mysql -c {SINGER_DATA}mysql_config.json --properties {SINGER_DATA}db.json --state {SINGER_DATA}state.json | ~/.virtualenvs/target-csv/bin/target-csv"
# 	bash_cmd=f"python {SINGER_DATA}csv_gsheet.py | ~/.virtualenvs/target-gsheet/bin/target-gsheet -c {SINGER_DATA}gsheet_config.json >> {SINGER_DATA}state.json"
# 	#run bash command
# 	os.system(bash_cmd)
