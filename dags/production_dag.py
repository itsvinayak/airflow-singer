...
def tap_mysql_target_csv():
    bash_cmd=f"{SINGER_DATA}/tap-mysql/bin/tap-mysql -c {SINGER_DATA}mysql_config.json --properties {SINGER_DATA}metaorigin_properties.json | {SINGER_DATA}/target-csv/bin/target-csv"
    print(os.getcwd())
...
def push_2_target():
    bash_cmd = f" {SINGER_DATA}/target-postgres/bin/python {SINGER_DATA}tap-csv.py | {SINGER_DATA}/target-postgres/bin/target-postgres --config {SINGER_DATA}target_postgres_config.json >> {SINGER_DATA}state.json"
    os.system(bash_cmd)
    print('push_test')
